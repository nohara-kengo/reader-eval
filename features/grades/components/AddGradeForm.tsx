...
import { addGradeAction } from "@/features/grades/server/actions";
import type { GradeFormState } from "@/features/grades/types";

const INITIAL_STATE: GradeFormState = { ok: false };

// 等級マスタへの追加フォーム。サーバ検証の結果（フィールド別エラー）を表示する。
export function AddGradeForm() {
  const [state, formAction, pending] = useActionState(addGradeAction, INITIAL_STATE);

  return (
    <form action={formAction} className="flex flex-col gap-4 rounded-lg border border-gray-200 p-5">
      <h2 className="text-base font-semibold">等級を追加</h2>
