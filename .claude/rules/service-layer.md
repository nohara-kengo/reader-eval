...
- サービスは**純粋なモジュール関数**を基本とする。副作用（DB / Entra）は**引数で渡されたクライアント**、または `lib/` の確立済みクライアント経由で行う（テストでモック差し替え可能に）
...
- **トランザクション内で非 DB の外部 I/O を行わない**（メール送信・ファイル出力等の外部呼び出し）。ロールバック不能になり、ロックも長引く
